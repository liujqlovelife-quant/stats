...
$$
E[Y(1)-Y(0)\mid X]-\operatorname{Cost}(X),
$$
...
$$
Y\mid X\sim\operatorname{Gamma}
...
$$
E[Y\mid X]=\mu,\qquad
\operatorname{Var}(Y\mid X)=\phi\mu^2.
$$

$\phi$ 固定时，变异系数 $\operatorname{SD}(Y\mid X)/E[Y\mid X]=\sqrt\phi$。
...
$$
\operatorname{Median}(Y\mid X)=e^{a(X)},\qquad
...
$$
\operatorname{Var}(Y\mid X)=\phi\mu^p.
$$
...
Y=\sum_{j=1}^{N}A_j,\qquad A_j\sim\operatorname{Gamma}.
$$
...
$$
s_g\mid p_g\sim\operatorname{Binomial}(n_g,p_g),
\qquad p_g\sim\operatorname{Beta}(a,b),
$$
...
+
\operatorname{Cov}(p_g,\mu_{+,g}\mid D).
$$
...
=
\operatorname{logit}(p_s)+\log\frac{s_0}{s_1}.
$$
...
+\operatorname{Cov}(I_t,C_t\mid A_t=1,X)
}{1000}.
$$
...
$$
\operatorname{BiasRatio}
=
\frac{\sum_i\hat y_i}{\sum_i y_i},
\qquad
\operatorname{RelativeBias}
=
...
\operatorname{Lift}@k
=
\frac{\operatorname{RevenueCapture}@k}{|S_k|/n}.
$$
